...
// NodePublic is the public portion of a a NodePrivate.
...
func (k *NodePublic) ReadFrom(r io.Reader) (int64, error) {
	var z NodePublic
	if *k != z {
		return 0, errors.New("refusing to read into non-zero NodePublic")
	}
	n, err := io.ReadFull(r, k.k[:])
	return int64(n), err
}

func (k NodePublic) WriteTo(w io.Writer) (int64, error) {
	n, err := w.Write(k.k[:])
	return int64(n), err
}
